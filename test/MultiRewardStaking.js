...
const { getSignatureFromTypedData, buildPermitParams, MAX_UINT_AMOUNT} = require('./helper')
...
describe('MultiRewardStaking', function () {
    ...
    beforeEach(async function () {
        ...
        let UnboundStaking = await ethers.getContractFactory('MultiRewardStaking')
    ...
    describe('#depositWithPermit', async () => {
        ...
        beforeEach(async function () {
            ...
            ownerPkey = "0x9d297c3cdf8af0abffbf00db443d56a62798d1d562ae19a668ac73eb9052f631";
            // mint unb to account for tests
            await unboundToken.mint(
                [{
                    dest: signers[0].address,
                    amount: "100"
                },
                {
                    dest: signers[1].address,
        ...
        it('should revert if permit expiration is invalid', async function () {
            ...
            const expiration = 0
            const nonce = (await unboundToken.nonces(signers[0].address)).toString()
            ...
            const msgParams = buildPermitParams(
              ...
              unboundToken.address,
              signers[0].address,
              unboundStaking.address,
              nonce,
              permitAmount,
              expiration
            )
            const { v, r, s } = getSignatureFromTypedData(ownerPkey, msgParams)
            ...
            await expect(unboundStaking.connect(signers[0]).depositWithPermit(0, permitAmount, expiration, v, r, s))
                .to.be.revertedWith("ERC20Permit: expired deadline");

        });

        it("should revert if owner doesn't have sufficient staking token balance", async function () {
            const { chainId } = await ethers.provider.getNetwork()

            const expiration = MAX_UINT_AMOUNT
            const nonce = (await unboundToken.nonces(signers[0].address)).toString()
            const permitAmount = "101"
        ...
        it('should update user staked amount', async function () {
            ...
            let userInfoBefore = await unboundStaking.getUserInfo(0, signers[0].address);

            expect(userInfoBefore.amount).to.be.equal(0);

            await unboundStaking.connect(signers[0]).depositWithPermit(0, permitAmount, expiration, v, r, s);

            let userInfoAfter = await unboundStaking.getUserInfo(0, signers[0].address);

            expect(userInfoAfter.amount).to.be.equal(permitAmount);
        ...
        it('should update total staked amount', async function () {
            ...
            expect(poolInfoAfter.totalStake).to.be.equal(permitAmount);
        ...
        it('should transfer token from user account to staking contract', async function () {
            ...
            expect(await unboundToken.balanceOf(signers[0].address)).to.be.equal("100");
            expect(await unboundToken.balanceOf(unboundStaking.address)).to.be.equal(totalRewardAmount);
            ...
            await expect(unboundStaking.connect(signers[0]).depositWithPermit(0, permitAmount, expiration, v, r, s))
                .to.emit(unboundToken, "Transfer")
                .withArgs(signers[0].address, unboundStaking.address, permitAmount);

            expect(await unboundToken.balanceOf(signers[0].address)).to.be.equal("0");

            let contractBalance = (new BigNumber(totalRewardAmount).plus(permitAmount)).toFixed();
            expect(await unboundToken.balanceOf(unboundStaking.address)).to.be.equal(contractBalance);
        ...
        it('should emit Deposit event', async function () {
            ...
            const { v, r, s } = getSignatureFromTypedData(ownerPkey, msgParams)
            let deposit = await unboundStaking.connect(signers[0]).depositWithPermit(0, permitAmount, expiration, v, r, s);

            await expect(deposit).to.emit(unboundStaking, "Deposit")
                .withArgs(signers[0].address, 0, deposit.blockNumber, permitAmount);
    ...
    describe('#harvest', async () => {
        ...
        it('should update user rewards', async function () {
            ...
            expect(userInfoAfter2.unclaimedRewards[0]).to.be.equal(0);
        ...
        it('should transfer rewards linearly & emit harvest event', async function () {
            ...
            await expect(harvest1).to.emit(unboundStaking, "Harvest")
                .withArgs(signers[1].address, 0, unboundToken.address, "1", harvest1.blockNumber)

            await expect(harvest2).to.emit(unboundStaking, "Harvest")
                .withArgs(signers[2].address, 0, unboundToken.address, "5", harvest2.blockNumber)

            await expect(harvest3).to.emit(unboundStaking, "Harvest")
                .withArgs(signers[3].address, 0, unboundToken.address, "12", harvest3.blockNumber) 
            
            await mineBlocks(500); // mine few more blocks
            
            let harvest4 = await unboundStaking.connect(signers[1]).harvest(0);
            let harvest5 = await unboundStaking.connect(signers[2]).harvest(0);
            let harvest6 = await unboundStaking.connect(signers[3]).harvest(0);
            
            await expect(harvest4).to.emit(unboundStaking, "Harvest")
                .withArgs(signers[1].address, 0, unboundToken.address, "50", harvest4.blockNumber)

            await expect(harvest5).to.emit(unboundStaking, "Harvest")
                .withArgs(signers[2].address, 0, unboundToken.address, "150", harvest5.blockNumber)

            await expect(harvest6).to.emit(unboundStaking, "Harvest")
                .withArgs(signers[3].address, 0, unboundToken.address, "301", harvest6.blockNumber) 
